...
impl BookingRepository for SqliteBookingRepo {
    ...
    async fn create_with_token(&self, booking: &Booking, token_to_burn: Option<String>, jobs: Vec<Job>) -> Result<Booking, AppError> {
        ...
        let created = sqlx::query_as::<_, Booking>("INSERT INTO bookings (id, tenant_id, event_id, invitee_id, start_time, end_time, customer_name, customer_email, customer_note, location, label_id, status, management_token, token, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *").bind(&booking.id).bind(&booking.tenant_id).bind(&booking.event_id).bind(&booking.invitee_id).bind(booking.start_time).bind(booking.end_time).bind(&booking.customer_name).bind(&booking.customer_email).bind(&booking.customer_note).bind(&booking.location).bind(&booking.label_id).bind(&booking.status).bind(&booking.management_token).bind(&booking.token).bind(booking.created_at).fetch_one(&mut *tx).await.map_err(AppError::Database)?;
        for job in jobs {
    ...
    async fn update(&self, booking: &Booking) -> Result<Booking, AppError> {
        sqlx::query_as::<_, Booking>("UPDATE bookings SET start_time=?, end_time=?, customer_name=?, customer_email=?, location=?, label_id=?, token=? WHERE id=? AND tenant_id=? RETURNING *").bind(booking.start_time).bind(booking.end_time).bind(&booking.customer_name).bind(&booking.customer_email).bind(&booking.location).bind(&booking.label_id).bind(&booking.token).bind(&booking.id).bind(&booking.tenant_id).fetch_one(&self.pool).await.map_err(AppError::Database)
    }
